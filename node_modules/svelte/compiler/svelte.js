...
function readExpression$1(parser, start, quoteMark) {
    ...
    var expression = parseExpressionAt(spaces(start) + str, start);
...
function readEventHandlerDirective(parser, start, name) {
    ...
    var expression = readExpression$1(parser, expressionStart, quoteMark);
...
function FuzzySet(arr, useLevenshtein, gramSizeLower, gramSizeUpper) {
    ...
    function levenshtein(str1, str2) {
        ...
        var prev;
        var value;
        for (var i_1 = 0; i_1 <= str2.length; i_1++) {
            for (var j = 0; j <= str1.length; j++) {
                if (i_1 && j) {
                    if (str1.charAt(j - 1) === str2.charAt(i_1 - 1)) {
                        value = prev;
                    }
                    else {
    ...
    function _iterateGrams(value, gramSize) {
        ...
        var results = [];
        if (lenDiff > 0) {
            for (var i_2 = 0; i_2 < lenDiff; ++i_2) {
                value += '-';
            }
        }
        for (var i_3 = 0; i_3 < simplified.length - gramSize + 1; ++i_3) {
            results.push(simplified.slice(i_3, i_3 + gramSize));
        }
        return results;
    }
    ...
    this.__get = function (value, gramSize) {
        ...
        if (this.useLevenshtein) {
            ...
            for (var i_4 = 0; i_4 < endIndex; ++i_4) {
                newResults_1.push([
                    _distance(results[i_4][1], normalizedValue),
                    results[i_4][1],
                ]);
            }
            results = newResults_1;
            results.sort(sortDescending);
        }
        var newResults = [];
        for (var i_5 = 0; i_5 < results.length; ++i_5) {
            if (results[i_5][0] == results[0][0]) {
                newResults.push([results[i_5][0], this.exactSet[results[i_5][1]]]);
            }
        }
        return newResults;
    };
...
function groupSelectors(selector) {
    ...
    selector.children.forEach(function (child, i) {
        if (child.type === 'WhiteSpace' || child.type === 'Combinator') {
            var next = selector.children[i + 1];
            block = {
...
function walkRules$1(nodes, callback) {
    nodes.forEach(function (node) {
        if (node.type === 'Rule') {
            ...
            if (node.name === 'media' || node.name === 'supports' || node.name === 'document') {
                walkRules$1(node.block.children, callback);
            }
        }
    });
}

function validateCss(validator, css) {
    walkRules$1(css.children, function (rule) {
        rule.selector.children.forEach(validateSelector);
    });
    function validateSelector(selector) {
        var blocks = groupSelectors(selector);
        blocks.forEach(function (block) {
            var i = block.selectors.length;
            while (i-- > 1) {
                var part = block.selectors[i];
        ...
        for (; start < end; start += 1) {
            if (!blocks[start].global)
                break;
        }
        for (; end > start; end -= 1) {
            if (!blocks[end - 1].global)
                break;
        }
...
var CodeBuilder = (function () {
    ...
    CodeBuilder.prototype.addConditionalLine = function (condition, line) {
...
function visitEachBlock(generator, block, state, node, elementStack) {
    ...
    if (node.needsAnchor) {
        block.addElement(anchor, "@createComment()", "@createComment()", state.parentNode, true);
    }
    else if (node.next) {
    ...
    node.children.forEach(function (child) {
        visit$1(generator, node._block, node._state, child, elementStack);
    });
    if (node["else"]) {
        node["else"].children.forEach(function (child) {
            visit$1(generator, node["else"]._block, node["else"]._state, child, elementStack);
        });
    }
    var _a, _b, _c, _d, _e;
}
function keyed(generator, block, state, node, snippet, _a) {
    ...
    if (node.children[0] && node.children[0].type === 'Element') {
        ...
        node._block.addElement(node._block.first, "@createComment()", "@createComment()", null, true);
    }
...
function unkeyed(generator, block, state, node, snippet, _a) {
    ...
    var condition = Array.from(allDependencies)
        .map(function (dependency) { return "'" + dependency + "' in changed"; })
        .join(' || ');
    var parentNode = state.parentNode || anchor + ".parentNode";
    if (condition !== '') {
        var forLoopBody = node._block.hasUpdateMethod
            ...
            : (_h = ["\n\t\t\t\t", "[#i] = ", "( ", ", ", ", ", "[#i], #i, #component );\n\t\t\t\t", "[#i].", "( ", ", ", " );\n\t\t\t"], _h.raw = ["\n\t\t\t\t", "[#i] = ", "( ", ", ", ", ", "[#i], #i, #component );\n\t\t\t\t", "[#i].", "( ", ", ", " );\n\t\t\t"], deindent(_h, iterations, create_each_block, params, each_block_value, each_block_value, iterations, mountOrIntro, parentNode, anchor));
...
function visitAttribute(generator, block, state, node, attribute, local) {
    if (attribute.value === true) {
        ...
        if (value.type === 'Text') {
            // static attributes
            var result = isNaN(value.data) ? stringify(value.data) : value.data;
            local.staticAttributes.push({
                name: attribute.name,
                value: result
            });
            ...
            var _a = block.contextualise(value.expression), dependencies = _a.dependencies, snippet = _a.snippet;
        ...
        var value = (attribute.value[0].type === 'Text' ? '' : "\"\" + ") +
            attribute.value
                ...
                if (chunk.type === 'Text') {
                    ...
                    dependencies.forEach(function (dependency) {
                        if (!~allDependencies_1.indexOf(dependency))
                            allDependencies_1.push(dependency);
                    });
                    return "( " + snippet + " )";
                }
            })
        ...
        local.dynamicAttributes.push({
            name: attribute.name,
            value: value,
            dependencies: allDependencies_1
        });
    }
}
...
function visitEventHandler(generator, block, state, node, attribute, local) {
    ...
    var declarations = usedContexts.map(function (name) {
        if (name === 'state')
            return 'var state = this._context.state;';
    ...
    local.create.addBlock((_a = ["\n\t\t", ".on( '", "', function ( event ) {\n\t\t\t", "\n\t\t});\n\t"], _a.raw = ["\n\t\t", ".on( '", "', function ( event ) {\n\t\t\t", "\n\t\t});\n\t"], deindent(_a, local.name, attribute.name, handlerBody)));
    var _a;
}
...
function getSetter(_a) {
    var block = _a.block, name = _a.name, snippet = _a.snippet, _this = _a._this, props = _a.props, attribute = _a.attribute, dependencies = _a.dependencies, value = _a.value;
    var tail = attribute.value.type === 'MemberExpression'
        ? getTailSnippet(attribute.value)
        : '';
    if (block.contexts.has(name)) {
        var prop = dependencies[0];
        var computed = isComputed(attribute.value);
        return (_b = ["\n\t\t\tvar list = ", ".", ".", ";\n\t\t\tvar index = ", ".", ".", ";\n\t\t\t", "\n\t\t\tlist[index]", " = ", ";\n\n\t\t\t", "\n\t\t"], _b.raw = ["\n\t\t\tvar list = ", ".", ".", ";\n\t\t\tvar index = ", ".", ".", ";\n\t\t\t", "\n\t\t\tlist[index]", " = ", ";\n\n\t\t\t",
            "\n\t\t"], deindent(_b, _this, props, block.listNames.get(name), _this, props, block.indexNames.get(name), computed && "var state = #component.get();", tail, value, computed
    ...
    if (attribute.value.type === 'MemberExpression') {
        return (_c = ["\n\t\t\tvar state = #component.get();\n\t\t\t", " = ", ";\n\t\t\t#component._set({ ", " });\n\t\t"], _c.raw = ["\n\t\t\tvar state = #component.get();\n\t\t\t", " = ", ";\n\t\t\t#component._set({ ",
            " });\n\t\t"], deindent(_c, snippet, value, dependencies
            .map(function (prop) { return prop + ": state." + prop; })
            .join(', ')));
    }
...
function visitBinding(generator, block, state, node, attribute, local) {
    ...
    var updating = block.getUniqueName(local.name + "_updating");
    block.addVariable(updating, 'false');
    local.create.addBlock((_b = ["\n\t\t#component._bindings.push( function () {\n\t\t\tif ( ", "._torndown ) return;\n\t\t\t", ".observe( '", "', function ( value ) {\n\t\t\t\tif ( ", " ) return;\n\t\t\t\t", " = true;\n\t\t\t\t", "\n\t\t\t\t", " = false;\n\t\t\t}, { init: @differs( ", ".get( '", "' ), ", " ) });\n\t\t});\n\t"], _b.raw = ["\n\t\t#component._bindings.push( function () {\n\t\t\tif ( ", "._torndown ) return;\n\t\t\t", ".observe( '", "', function ( value ) {\n\t\t\t\tif ( ", " ) return;\n\t\t\t\t", " = true;\n\t\t\t\t", "\n\t\t\t\t", " = false;\n\t\t\t}, { init: @differs( ", ".get( '", "' ), ", " ) });\n\t\t});\n\t"], deindent(_b, local.name, local.name, attribute.name, updating, updating, setter, updating, local.name, attribute.name, snippet)));
    local.update.addBlock((_c = ["\n\t\tif ( !", " && ", " ) {\n\t\t\t", " = true;\n\t\t\t", "._set({ ", ": ", " });\n\t\t\t", " = false;\n\t\t}\n\t"], _c.raw = ["\n\t\tif ( !", " && ",
        " ) {\n\t\t\t", " = true;\n\t\t\t", "._set({ ", ": ", " });\n\t\t\t", " = false;\n\t\t}\n\t"], deindent(_c, updating, dependencies
        .map(function (dependency) { return "'" + dependency + "' in changed"; })
        .join(' || '), updating, local.name, attribute.name, snippet, updating)));
    var _b, _c;
}
...
function visitComponent(generator, block, state, node, elementStack) {
    ...
    node.attributes
        .sort(function (a, b) { return order$1[a.type] - order$1[b.type]; })
        .forEach(function (attribute) {
        visitors$2[attribute.type](generator, block, childState, node, attribute, local);
    });
    if (local.allUsedContexts.length) {
        var initialProps = local.allUsedContexts
            ...
            .join(',\n');
        var updates = local.allUsedContexts
            .map(function (contextName) {
            if (contextName === 'state')
                return name + "._context.state = state;";
            var listName = block.listNames.get(contextName);
            var indexName = block.indexNames.get(contextName);
            return name + "._context." + listName + " = " + listName + ";\n" + name + "._context." + indexName + " = " + indexName + ";";
        })
            .join('\n');
        local.create.addBlock((_a = ["\n\t\t\t", "._context = {\n\t\t\t\t", "\n\t\t\t};\n\t\t"], _a.raw = ["\n\t\t\t", "._context = {\n\t\t\t\t", "\n\t\t\t};\n\t\t"], deindent(_a, name, initialProps)));
    ...
    if (hasChildren) {
        var params = block.params.join(', ');
        var childBlock_1 = node._block;
        node.children.forEach(function (child) {
            visit$1(generator, childBlock_1, childState, child, elementStack);
        });
        var yieldFragment = block.getUniqueName(name + "_yield_fragment");
        block.builders.init.addLine("var " + yieldFragment + " = " + childBlock_1.name + "( " + params + ", #component );");
        block.builders.create.addLine(yieldFragment + ".create();");
        block.builders.claim.addLine(yieldFragment + ".claim( " + state.parentNodes + " );");
        if (childBlock_1.hasUpdateMethod) {
            block.builders.update.addLine(yieldFragment + ".update( changed, " + params + " );");
        }
    ...
    if (local.staticAttributes.length ||
        ...
        var initialProps = local.staticAttributes
            .concat(local.dynamicAttributes)
            .map(function (attribute) { return attribute.name + ": " + attribute.value; });
        var initialPropString = stringifyProps(initialProps);
        if (local.bindings.length) {
            var initialData_1 = block.getUniqueName(name + "_initial_data");
            statements.push("var " + initialData_1 + " = " + initialPropString + ";");
            local.bindings.forEach(function (binding) {
                statements.push("if ( " + binding.prop + " in " + binding.obj + " ) " + initialData_1 + "." + binding.name + " = " + binding.value + ";");
            });
            componentInitProperties.push("data: " + initialData_1);
        }
    ...
    local.create.addBlockAtStart((_b = ["\n\t\t", "\n\t\tvar ", " = new ", "({\n\t\t\t", "\n\t\t});\n\t"], _b.raw = ["\n\t\t", "\n\t\tvar ", " = new ", "({\n\t\t\t", "\n\t\t});\n\t"], deindent(_b, statements.join('\n'), name, expression, componentInitProperties.join(',\n'))));
    if (local.dynamicAttributes.length) {
        var updates = local.dynamicAttributes.map(function (attribute) {
    ...
    if (!local.update.isEmpty())
        block.builders.update.addBlock(local.update);
    var _a, _b, _c;
}
...
function visitAttribute$1(generator, block, state, node, attribute) {
    ...
    if (isDynamic) {
        ...
        if (attribute.value.length === 1) {
            ...
            var snippet = block.contextualise(attribute.value[0].expression).snippet;
            value = snippet;
        }
        else {
        ...
        var last = block.getUniqueName(state.parentNode + "_" + name.replace(/[^a-zA-Z_$]/g, '_') + "_value");
        block.addVariable(last);
        ...
        if (isSelectValueAttribute) {
            ...
            var ifStatement = isMultipleSelect
                ? (_a = ["\n\t\t\t\t\t", ".selected = ~", ".indexOf( ", ".__value );"], _a.raw = ["\n\t\t\t\t\t", ".selected = ~", ".indexOf( ", ".__value );"], deindent(_a, option, last, option)) : (_b = ["\n\t\t\t\t\tif ( ", ".__value === ", " ) {\n\t\t\t\t\t\t", ".selected = true;\n\t\t\t\t\t\tbreak;\n\t\t\t\t\t}"], _b.raw = ["\n\t\t\t\t\tif ( ", ".__value === ", " ) {\n\t\t\t\t\t\t", ".selected = true;\n\t\t\t\t\t\tbreak;\n\t\t\t\t\t}"], deindent(_b, option, last, option));
            updater = (_c = ["\n\t\t\t\tfor ( var ", " = 0; ", " < ", ".options.length; ", " += 1 ) {\n\t\t\t\t\tvar ", " = ", ".options[", "];\n\n\t\t\t\t\t", "\n\t\t\t\t}\n\t\t\t"], _c.raw = ["\n\t\t\t\tfor ( var ", " = 0; ", " < ", ".options.length; ", " += 1 ) {\n\t\t\t\t\tvar ", " = ", ".options[", "];\n\n\t\t\t\t\t", "\n\t\t\t\t}\n\t\t\t"], deindent(_c, i, i, state.parentNode, i, option, state.parentNode, i, ifStatement));
            block.builders.hydrate.addLine((_d = ["\n\t\t\t\t", " = ", "\n\t\t\t\t", "\n\t\t\t"], _d.raw = ["\n\t\t\t\t", " = ", "\n\t\t\t\t", "\n\t\t\t"], deindent(_d, last, value, updater)));
        }
        else if (propertyName) {
            block.builders.hydrate.addLine(state.parentNode + "." + propertyName + " = " + last + " = " + value + ";");
            updater = state.parentNode + "." + propertyName + " = " + last + ";";
        }
        else {
            block.builders.hydrate.addLine(method + "( " + state.parentNode + ", '" + name + "', " + last + " = " + value + " );");
            updater = method + "( " + state.parentNode + ", '" + name + "', " + last + " );";
        }
        block.builders.update.addBlock((_e = ["\n\t\t\tif ( ", " !== ( ", " = ", " ) ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _e.raw = ["\n\t\t\tif ( ", " !== ( ", " = ", " ) ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_e, last, last, value, updater)));
    }
    else {
...
function visitEventHandler$1(generator, block, state, node, attribute) {
    ...
    var handlerBody = (_a = ["\n\t\t", "\n\t\t", "\n\t\t[\u2702", "-", "\u2702];\n\t"], _a.raw = ["\n\t\t",
        ...
        "var " + block.alias('component') + " = " + _this + "._svelte.component;", declarations, attribute.expression.start, attribute.expression.end));
    if (isCustomEvent) {
        ...
        if (shouldHoist) {
            generator.blocks.push({
                render: function () { return handler_1; }
            });
        }
        else {
            block.builders.init.addBlock(handler_1);
        }
...
function visitBinding$1(generator, block, state, node, attribute) {
    ...
    if (node.name === 'select') {
        ...
        generator.hasComplexBindings = true;
        block.builders.hydrate.addBlock("if ( !('" + name + "' in state) ) #component._bindings.push( " + handler + " );");
    }
    ...
    block.builders.hydrate.addBlock("@addListener( " + state.parentNode + ", '" + eventName + "', " + handler + " );");
    if (node.name !== 'audio' && node.name !== 'video')
        node.initialUpdate = updateElement;
    if (updateCondition !== null) {
        // audio/video duration is read-only, it never updates
        block.builders.update.addBlock((_h = ["\n\t\t\tif ( ", " ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _h.raw = ["\n\t\t\tif ( ", " ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_h, updateCondition, updateElement)));
    }
...
function visitElement(generator, block, state, node, elementStack) {
    ...
    node.children.forEach(function (child) {
        visit$1(generator, block, childState, child, elementStack.concat(node));
    });
    if (node.lateUpdate) {
...
function getBranches(generator, block, state, node, elementStack) {
    var branches = [
        {
    ...
    if (isElseIf(node["else"])) {
        branches.push.apply(branches, getBranches(generator, block, state, node["else"].children[0], elementStack));
    }
    else {
        branches.push({
            condition: null,
...
function visitChildren(generator, block, state, node, elementStack) {
    node.children.forEach(function (child) {
        visit$1(generator, node._block, node._state, child, elementStack);
    });
}
function visitIfBlock(generator, block, state, node, elementStack) {
    var name = generator.getUniqueName("if_block");
    var anchor = node.needsAnchor
        ? block.getUniqueName(name + "_anchor")
        : (node.next && node.next._state.name) || 'null';
    var params = block.params.join(', ');
    var branches = getBranches(generator, block, state, node, elementStack);
...
function compound(generator, block, state, node, branches, dynamic, _a) {
    ...
    var get_block = block.getUniqueName("get_block");
    var current_block = block.getUniqueName("current_block");
    var current_block_and = hasElse ? '' : current_block + " && ";
    block.builders.init.addBlock((_b = ["\n\t\tfunction ", " ( ", " ) {\n\t\t\t", "\n\t\t}\n\n\t\tvar ", " = ", "( ", " );\n\t\tvar ", " = ", "", "( ", ", #component );\n\t"], _b.raw = ["\n\t\tfunction ", " ( ", " ) {\n\t\t\t",
        "\n\t\t}\n\n\t\tvar ", " = ", "( ", " );\n\t\tvar ", " = ", "", "( ", ", #component );\n\t"], deindent(_b, get_block, params, branches
        .map(function (_a) {
        var condition = _a.condition, block = _a.block;
        return (condition ? "if ( " + condition + " ) " : '') + "return " + block + ";";
    })
        .join('\n'), current_block, get_block, params, name, current_block_and, current_block, params)));
    var isTopLevel = !state.parentNode;
    ...
    var changeBlock = (_c = ["\n\t\t", "\n\t\t", " = ", "", "( ", ", #component );\n\t\t", "", ".create();\n\t\t", "", ".", "( ", ", ", " );\n\t"], _c.raw = ["\n\t\t",
        "\n\t\t", " = ", "", "( ", ", #component );\n\t\t", "", ".create();\n\t\t", "", ".", "( ", ", ", " );\n\t"], deindent(_c, hasElse
        ? (_d = ["\n\t\t\t\t", ".unmount();\n\t\t\t\t", ".destroy();\n\t\t\t"], _d.raw = ["\n\t\t\t\t", ".unmount();\n\t\t\t\t", ".destroy();\n\t\t\t"], deindent(_d, name, name)) : (_e = ["\n\t\t\t\tif ( ", " ) {\n\t\t\t\t\t", ".unmount();\n\t\t\t\t\t", ".destroy();\n\t\t\t\t}"], _e.raw = ["\n\t\t\t\tif ( ", " ) {\n\t\t\t\t\t", ".unmount();\n\t\t\t\t\t", ".destroy();\n\t\t\t\t}"], deindent(_e, name, name, name)), name, current_block_and, current_block, params, if_name, name, if_name, name, mountOrIntro, parentNode, anchor));
    if (dynamic) {
        block.builders.update.addBlock((_f = ["\n\t\t\tif ( ", " === ( ", " = ", "( ", " ) ) && ", " ) {\n\t\t\t\t", ".update( changed, ", " );\n\t\t\t} else {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _f.raw = ["\n\t\t\tif ( ", " === ( ", " = ", "( ", " ) ) && ", " ) {\n\t\t\t\t", ".update( changed, ", " );\n\t\t\t} else {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_f, current_block, current_block, get_block, params, name, name, params, changeBlock)));
    }
    else {
        block.builders.update.addBlock((_g = ["\n\t\t\tif ( ", " !== ( ", " = ", "( ", " ) ) ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _g.raw = ["\n\t\t\tif ( ", " !== ( ", " = ", "( ", " ) ) ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_g, current_block, current_block, get_block, params, changeBlock)));
    }
    block.builders.unmount.addLine("" + if_name + name + ".unmount();");
    block.builders.destroy.addLine("" + if_name + name + ".destroy();");
...
function compoundWithOutros(generator, block, state, node, branches, dynamic, _a) {
    ...
    var get_block = block.getUniqueName("get_block");
    var current_block_index = block.getUniqueName("current_block_index");
    ...
    block.builders.init.addBlock((_b = ["\n\t\tvar ", " = [\n\t\t\t", "\n\t\t];\n\n\t\tvar ", " = [];\n\n\t\tfunction ", " ( ", " ) {\n\t\t\t", "\n\t\t}\n\t"], _b.raw = ["\n\t\tvar ", " = [\n\t\t\t", "\n\t\t];\n\n\t\tvar ", " = [];\n\n\t\tfunction ", " ( ", " ) {\n\t\t\t",
        ...
        .join('\n'))));
    if (hasElse) {
        block.builders.init.addBlock((_c = ["\n\t\t\t", " = ", "( ", " );\n\t\t\t", " = ", "[ ", " ] = ", "[ ", " ]( ", ", #component );\n\t\t"], _c.raw = ["\n\t\t\t", " = ", "( ", " );\n\t\t\t", " = ", "[ ", " ] = ", "[ ", " ]( ", ", #component );\n\t\t"], deindent(_c, current_block_index, get_block, params, name, if_blocks, current_block_index, if_block_creators, current_block_index, params)));
    }
    else {
        block.builders.init.addBlock((_d = ["\n\t\t\tif ( ~( ", " = ", "( ", " ) ) ) {\n\t\t\t\t", " = ", "[ ", " ] = ", "[ ", " ]( ", ", #component );\n\t\t\t}\n\t\t"], _d.raw = ["\n\t\t\tif ( ~( ", " = ", "( ", " ) ) ) {\n\t\t\t\t", " = ", "[ ", " ] = ", "[ ", " ]( ", ", #component );\n\t\t\t}\n\t\t"], deindent(_d, current_block_index, get_block, params, name, if_blocks, current_block_index, if_block_creators, current_block_index, params)));
    }
    var isTopLevel = !state.parentNode;
    var mountOrIntro = branches[0].hasIntroMethod ? 'intro' : 'mount';
    var targetNode = state.parentNode || '#target';
    ...
    var createNewBlock = (_f = ["\n\t\t", " = ", "[ ", " ];\n\t\tif ( !", " ) {\n\t\t\t", " = ", "[ ", " ] = ", "[ ", " ]( ", ", #component );\n\t\t\t", ".create();\n\t\t}\n\t\t", ".", "( ", ", ", " );\n\t"], _f.raw = ["\n\t\t", " = ", "[ ", " ];\n\t\tif ( !", " ) {\n\t\t\t", " = ", "[ ", " ] = ", "[ ", " ]( ", ", #component );\n\t\t\t", ".create();\n\t\t}\n\t\t", ".", "( ", ", ", " );\n\t"], deindent(_f, name, if_blocks, current_block_index, name, name, if_blocks, current_block_index, if_block_creators, current_block_index, params, name, name, mountOrIntro, parentNode, anchor));
    var changeBlock = hasElse
        ? (_g = ["\n\t\t\t", "\n\n\t\t\t", "\n\t\t"], _g.raw = ["\n\t\t\t", "\n\n\t\t\t", "\n\t\t"], deindent(_g, destroyOldBlock, createNewBlock)) : (_h = ["\n\t\t\tif ( ", " ) {\n\t\t\t\t", "\n\t\t\t}\n\n\t\t\tif ( ~", " ) {\n\t\t\t\t", "\n\t\t\t} else {\n\t\t\t\t", " = null;\n\t\t\t}\n\t\t"], _h.raw = ["\n\t\t\tif ( ", " ) {\n\t\t\t\t", "\n\t\t\t}\n\n\t\t\tif ( ~", " ) {\n\t\t\t\t", "\n\t\t\t} else {\n\t\t\t\t", " = null;\n\t\t\t}\n\t\t"], deindent(_h, name, destroyOldBlock, current_block_index, createNewBlock, name));
    if (dynamic) {
        block.builders.update.addBlock((_j = ["\n\t\t\tvar ", " = ", ";\n\t\t\t", " = ", "( ", " );\n\t\t\tif ( ", " === ", " ) {\n\t\t\t\t", "", "[ ", " ].update( changed, ", " );\n\t\t\t} else {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _j.raw = ["\n\t\t\tvar ", " = ", ";\n\t\t\t", " = ", "( ", " );\n\t\t\tif ( ", " === ", " ) {\n\t\t\t\t", "", "[ ", " ].update( changed, ", " );\n\t\t\t} else {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_j, previous_block_index, current_block_index, current_block_index, get_block, params, current_block_index, previous_block_index, if_current_block_index, if_blocks, current_block_index, params, changeBlock)));
    }
    else {
        block.builders.update.addBlock((_k = ["\n\t\t\tvar ", " = ", ";\n\t\t\t", " = ", "( ", " );\n\t\t\tif ( ", " !== ", " ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _k.raw = ["\n\t\t\tvar ", " = ", ";\n\t\t\t", " = ", "( ", " );\n\t\t\tif ( ", " !== ", " ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_k, previous_block_index, current_block_index, current_block_index, get_block, params, current_block_index, previous_block_index, changeBlock)));
    }
    block.builders.destroy.addLine((_l = ["\n\t\t", "{\n\t\t\t", "[ ", " ].unmount();\n\t\t\t", "[ ", " ].destroy();\n\t\t}\n\t"], _l.raw = ["\n\t\t", "{\n\t\t\t", "[ ", " ].unmount();\n\t\t\t", "[ ", " ].destroy();\n\t\t}\n\t"], deindent(_l, if_current_block_index, if_blocks, current_block_index, if_blocks, current_block_index)));
    var _b, _c, _d, _e, _f, _g, _h, _j, _k, _l;
}

function visitMustacheTag(generator, block, state, node) {
    var name = node._state.name;
    var value = block.getUniqueName(name + "_value");
    var snippet = block.contextualise(node.expression).snippet;
    block.addVariable(value);
    block.addElement(name, "@createText( " + value + " = " + snippet + " )", generator.hydratable
        ? "@claimText( " + state.parentNodes + ", " + value + " = " + snippet + " )"
        : '', state.parentNode, true);
    block.builders.update.addBlock((_a = ["\n\t\tif ( ", " !== ( ", " = ", " ) ) {\n\t\t\t", ".data = ", ";\n\t\t}\n\t"], _a.raw = ["\n\t\tif ( ", " !== ( ", " = ", " ) ) {\n\t\t\t", ".data = ", ";\n\t\t}\n\t"], deindent(_a, value, value, snippet, name, value)));
    var _a;
}
...
function visitRawMustacheTag(generator, block, state, node) {
    ...
    block.addElement(before, "@createElement( 'noscript' )", "@createElement( 'noscript' )", state.parentNode, true);
    block.addElement(after, "@createElement( 'noscript' )", "@createElement( 'noscript' )", state.parentNode, true);
    var isToplevel = !state.parentNode;
    var mountStatement = before + ".insertAdjacentHTML( 'afterend', " + value + " = " + snippet + " );";
    var detachStatement = "@detachBetween( " + before + ", " + after + " );";
    block.builders.mount.addLine(mountStatement);
    block.builders.update.addBlock((_a = ["\n\t\tif ( ", " !== ( ", " = ", " ) ) {\n\t\t\t", "\n\t\t\t", "\n\t\t}\n\t"], _a.raw = ["\n\t\tif ( ", " !== ( ", " = ", " ) ) {\n\t\t\t", "\n\t\t\t", "\n\t\t}\n\t"], deindent(_a, value, value, snippet, detachStatement, mountStatement)));
...
function visitText(generator, block, state, node) {
    if (!node._state.shouldCreate)
        return;
    block.addElement(node._state.name, "@createText( " + stringify(node.data) + " )", generator.hydratable
        ? "@claimText( " + state.parentNodes + ", " + stringify(node.data) + " )"
        : '', state.parentNode, node.usedAsAnchor);
}
...
function visit$1(generator, block, state, node, elementStack) {
...
function processCss(generator, code, cascade) {
    var css = generator.parsed.css.content.styles;
    var offset = generator.parsed.css.content.start;
    var attr = "[svelte-" + generator.parsed.hash + "]";
            ...
            node.expression.children.forEach(function (expression) {
                if (expression.type === 'Identifier') {
                    if (expression.name.startsWith('-global-')) {
                        ...
                        var newName = "svelte-" + generator.parsed.hash + "-" + expression.name;
        ...
        rule.selector.children.forEach(function (selector) {
            if (cascade) {
                ...
                if (firstToken.type === 'TypeSelector') {
                    ...
                    var head = firstToken.name === '*' ? css.slice(firstToken.end - offset, insert) : css.slice(start, insert);
    ...
    walkRules$1(generator.parsed.css.children, transform);
    if (!cascade) {
        generator.selectors.forEach(function (selector) {
...
var Selector$2 = (function () {
    function Selector(node) {
        ...
        while (i > 2) {
            ...
            var penultimate = node.children[i - 2];
    ...
    Selector.prototype.apply = function (node, stack) {
        var applies = selectorAppliesTo(this.parts, node, stack.slice());
        if (applies) {
            this.used = true;
            ...
            if (stack[0] && this.node.children.find(isDescendantSelector))
                stack[0]._needsCssAttribute = true;
        }
    };
    Selector.prototype.transform = function (code, attr) {
        ...
        this.blocks.forEach(function (block, i) {
            ...
            else if (i === 0 || i === _this.blocks.length - 1) {
...
function selectorAppliesTo(parts, node, stack) {
    var i = parts.length;
    var j = stack.length;
    while (i--) {
        if (!node) {
        ...
        if (part.type === 'ClassSelector') {
            if (!attributeMatches(node, 'class', part.name, '~=', false))
                return false;
        }
        else if (part.type === 'IdSelector') {
            if (!attributeMatches(node, 'id', part.name, '=', false))
                return false;
        }
        else if (part.type === 'AttributeSelector') {
            if (!attributeMatches(node, part.name.name, part.value && unquote(part.value.value), part.operator, part.flags))
                return false;
        }
...
function attributeMatches(node, name, expectedValue, operator, caseInsensitive) {
    ...
    if (isDynamic(attr.value))
        return true;
    var actualValue = attr.value[0].data;
...
var Generator = (function () {
    function Generator(parsed, source, name, options) {
        var _this = this;
        ...
        this.selectors = [];
        if (parsed.css) {
            walkRules$1(parsed.css.children, function (node) {
                node.selector.children.forEach(function (child) {
                    _this.selectors.push(new Selector$2(child));
                });
            });
            ...
            enter: function (node, parent, key) {
                ...
                if (node.type === 'ThisExpression') {
                    ...
                    if (name === 'event' && isEventHandler) {
                        ...
                        if (!~usedContexts.indexOf(name))
                            usedContexts.push(name);
                    }
                        ...
                        if (!~usedContexts.indexOf(context_1))
                            usedContexts.push(context_1);
                    }
                    else {
                        ...
                        if (!~usedContexts.indexOf('state'))
                            usedContexts.push('state');
                    }
                    this.skip();
                }
            },
    ...
    Generator.prototype.applyCss = function (node, stack) {
        if (!this.cssId)
            return;
        if (this.cascade) {
            if (stack.length === 0)
    ...
    Generator.prototype.warnOnUnusedSelectors = function () {
        var _this = this;
        if (this.cascade)
            return;
        var locator;
        this.selectors.forEach(function (selector) {
            if (!selector.used) {
                ...
                var _a = locator(pos), line_1 = _a.line, column_1 = _a.column;
                var frame_1 = getCodeFrame(_this.source, line_1, column_1);
                var message_1 = "Unused CSS selector";
                _this.options.onwarn({
                    message: message_1,
                    frame: frame_1,
                    loc: { line: line_1 + 1, column: column_1 },
                    pos: pos,
                    filename: _this.options.filename,
                    toString: function () { return message_1 + " (" + (line_1 + 1) + ":" + column_1 + ")\n" + frame_1; }
                });
            }
        });
    };
...
var Block$2 = (function () {
    ...
    Block.prototype.addElement = function (name, renderStatement, claimStatement, parentNode, needsIdentifier) {
    ...
    Block.prototype.render = function () {
        ...
        return (_m = ["\n\t\t\tfunction ", " ( ", ", #component", " ) {\n\t\t\t\t", "\n\n\t\t\t\t", "\n\n\t\t\t\treturn {\n\t\t\t\t\t", "\n\t\t\t\t};\n\t\t\t}\n\t\t"], _m.raw = ["\n\t\t\tfunction ", " ( ", ", #component",
            ...
            "var " + Array.from(this.variables.keys())
                ...
                .join(', ') + ";", !this.builders.init.isEmpty() && this.builders.init, properties)).replace(/(\\)?#(\w*)/g, function (match, escaped, name) {
            return escaped ? match.slice(1) : _this.alias(name);
        });
...
var preprocessors = {
    MustacheTag: function (generator, block, state, node, elementStack, stripWhitespace) {
        var dependencies = block.findDependencies(node.expression);
    ...
    RawMustacheTag: function (generator, block, state, node, elementStack, stripWhitespace) {
        var dependencies = block.findDependencies(node.expression);
        block.addDependencies(dependencies);
    ...
    Text: function (generator, block, state, node, elementStack, stripWhitespace) {
        node._state = getChildState(state);
        if (!/\S/.test(node.data)) {
            if (state.namespace)
                return;
    ...
    IfBlock: function (generator, block, state, node, elementStack, stripWhitespace, nextSibling) {
        var blocks = [];
        var dynamic = false;
        var hasIntros = false;
        var hasOutros = false;
        function attachBlocks(node) {
            ...
            if (isElseIf$1(node["else"])) {
                ...
                preprocessChildren(generator, node["else"]._block, node["else"]._state, node["else"], elementStack, stripWhitespace, nextSibling);
    ...
    EachBlock: function (generator, block, state, node, elementStack, stripWhitespace, nextSibling) {
        var dependencies = block.findDependencies(node.expression);
        block.addDependencies(dependencies);
        ...
        if (node.index)
            indexes.set(indexName, node.context);
    ...
    Element: function (generator, block, state, node, elementStack, stripWhitespace, nextSibling) {
        node.attributes.forEach(function (attribute) {
        ...
        if (isComponent) {
            ...
            var name = block.getUniqueName(node.name.replace(/[^a-zA-Z0-9_$]/g, '_'));
            node._state = getChildState(state, {
                isTopLevel: false,
                name: name,
                parentNode: name,
...
function preprocessChildren(generator, block, state, node, elementStack, stripWhitespace, nextSibling) {
    ...
    cleaned.forEach(function (child, i) {
        ...
        if (preprocessor)
            preprocessor(generator, block, state, child, elementStack, stripWhitespace, cleaned[i + 1] || nextSibling);
        if (lastChild) {
...
var DomGenerator = (function (_super) {
    ...
    function DomGenerator(parsed, source, name, options) {
        var _this = _super.call(this, parsed, source, name, options) || this;
        _this.blocks = [];
...
function dom(parsed, source, options) {
    ...
    var generator = new DomGenerator(parsed, source, options.name || 'SvelteComponent', options);
    ...
    if (computations.length) {
        var computationBuilder_1 = new CodeBuilder();
        computations.forEach(function (_a) {
            ...
            var condition = "isInitial || " + deps
                .map(function (dep) {
                return "( '" + dep + "' in newState && @differs( state." + dep + ", oldState." + dep + " ) )";
            })
                .join(' || ');
            var statement = "state." + key + " = newState." + key + " = @template.computed." + key + "( " + deps
                .map(function (dep) { return "state." + dep; })
                .join(', ') + " );";
            computationBuilder_1.addConditionalLine(condition, statement);
        });
        builder.addBlock((_b = ["\n\t\t\tfunction @recompute ( state, newState, oldState, isInitial ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], _b.raw = ["\n\t\t\tfunction @recompute ( state, newState, oldState, isInitial ) {\n\t\t\t\t", "\n\t\t\t}\n\t\t"], deindent(_b, computationBuilder_1)));
    }
    var _set = (_c = ["\n\t\t", "\n\n\t\tvar oldState = this._state;\n\t\tthis._state = @assign( {}, oldState, newState );\n\t\t", "\n\t\t@dispatchObservers( this, this._observers.pre, newState, oldState );\n\t\t", "\n\t\t@dispatchObservers( this, this._observers.post, newState, oldState );\n\t\t", "\n\t\t", "\n\t"], _c.raw = ["\n\t\t",
        "\n\n\t\tvar oldState = this._state;\n\t\tthis._state = @assign( {}, oldState, newState );\n\t\t",
        "\n\t\t@dispatchObservers( this, this._observers.pre, newState, oldState );\n\t\t", "\n\t\t@dispatchObservers( this, this._observers.post, newState, oldState );\n\t\t",
        "\n\t\t",
        "\n\t"], deindent(_c, options.dev && (_d = ["\n\t\t\tif ( typeof newState !== 'object' ) {\n\t\t\t\tthrow new Error( 'Component .set was called without an object of data key-values to update.' );\n\t\t\t}\n\n\t\t\t", "\n\t\t"], _d.raw = ["\n\t\t\tif ( typeof newState !== 'object' ) {\n\t\t\t\tthrow new Error( 'Component .set was called without an object of data key-values to update.' );\n\t\t\t}\n\n\t\t\t",
        "\n\t\t"], deindent(_d, Array.from(generator.readonly).map(function (prop) {
        return "if ( '" + prop + "' in newState && !this._updatingReadonlyProperty ) throw new Error( \"Cannot set read-only property '" + prop + "'\" );";
    }))), computations.length &&
        "@recompute( this._state, newState, oldState, false )", block.hasUpdateMethod && "this._fragment.update( newState, this._state );", (generator.hasComponents || generator.hasIntroTransitions) &&
        "this._flush();", generator.hasComplexBindings &&
        "while ( this._bindings.length ) this._bindings.pop()();"));
    if (hasJs) {
        builder.addBlock("[\u2702" + parsed.js.content.start + "-" + parsed.js.content.end + "\u2702]");
    }
    if (generator.css && options.css !== false) {
        builder.addBlock((_e = ["\n\t\t\tfunction @add_css () {\n\t\t\t\tvar style = @createElement( 'style' );\n\t\t\t\tstyle.id = '", "-style';\n\t\t\t\tstyle.textContent = ", ";\n\t\t\t\t@appendNode( style, document.head );\n\t\t\t}\n\t\t"], _e.raw = ["\n\t\t\tfunction @add_css () {\n\t\t\t\tvar style = @createElement( 'style' );\n\t\t\t\tstyle.id = '", "-style';\n\t\t\t\tstyle.textContent = ", ";\n\t\t\t\t@appendNode( style, document.head );\n\t\t\t}\n\t\t"], deindent(_e, generator.cssId, stringify(generator.css))));
    }
    ...
    var proto = sharedPath
        ? "@proto "
        : (_f = ["\n\t\t{\n\t\t\t", "\n\t\t}"], _f.raw = ["\n\t\t{\n\t\t\t",
            "\n\t\t}"], deindent(_f, ['get', 'fire', 'observe', 'on', 'set', '_flush']
    ...
    builder.addBlock((_g = ["\n\t\tfunction ", " ( options ) {\n\t\t\toptions = options || {};\n\t\t\t", "\n\t\t\t", "\n\t\t\tthis._state = ", ";\n\t\t\t", "\n\t\t\t", "\n\t\t\t", "\n\t\t\t", "\n\n\t\t\tthis._observers = {\n\t\t\t\tpre: Object.create( null ),\n\t\t\t\tpost: Object.create( null )\n\t\t\t};\n\n\t\t\tthis._handlers = Object.create( null );\n\n\t\t\tthis._root = options._root || this;\n\t\t\tthis._yield = options._yield;\n\n\t\t\tthis._torndown = false;\n\t\t\t", "\n\t\t\t", "\n\t\t\t", "\n\n\t\t\tthis._fragment = @create_main_fragment( this._state, this );\n\n\t\t\tif ( options.target ) {\n\t\t\t\t", "\n\t\t\t\tthis._fragment.", "( options.target, null );\n\t\t\t}\n\t\t\t\n\t\t\t", "\n\t\t\t", "\n\n\t\t\t", "\n\t\t}\n\n\t\t@assign( ", ", ", ");\n\n\t\t", ".prototype._set = function _set ( newState ) {\n\t\t\t", "\n\t\t};\n\n\t\t", ".prototype.teardown = ", ".prototype.destroy = function destroy ( detach ) {\n\t\t\tthis.fire( 'destroy' );\n\t\t\t", "\n\n\t\t\tif ( detach !== false ) this._fragment.unmount();\n\t\t\tthis._fragment.destroy();\n\t\t\tthis._fragment = null;\n\n\t\t\tthis._state = {};\n\t\t\tthis._torndown = true;\n\t\t};\n\t"], _g.raw = ["\n\t\tfunction ", " ( options ) {\n\t\t\toptions = options || {};\n\t\t\t",
        ...
        : "options.data || {}", generator.metaBindings, computations.length && "@recompute( this._state, this._state, {}, true );", options.dev &&
        ...
        "this._bindingGroups = [ " + Array(generator.bindingGroups.length)
            ...
            .join(', ') + " ];", generator.css &&
        options.css !== false &&
        "if ( !document.getElementById( '" + generator.cssId + "-style' ) ) @add_css();", (generator.hasComponents || generator.hasIntroTransitions) &&
        "this._oncreate = [];", generator.hasComplexBindings && "this._bindings = [];", generator.hydratable
        ? (_h = ["\n\t\t\t\t\t\tvar nodes = @children( options.target );\n\t\t\t\t\t\toptions.hydrate ? this._fragment.claim( nodes ) : this._fragment.create();\n\t\t\t\t\t\tnodes.forEach( @detachNode );\n\t\t\t\t\t"], _h.raw = ["\n\t\t\t\t\t\tvar nodes = @children( options.target );\n\t\t\t\t\t\toptions.hydrate ? this._fragment.claim( nodes ) : this._fragment.create();\n\t\t\t\t\t\tnodes.forEach( @detachNode );\n\t\t\t\t\t"], deindent(_h)) : (_j = ["\n\t\t\t\t\t\t", "\n\t\t\t\t\t\tthis._fragment.create();\n\t\t\t\t\t"], _j.raw = ["\n\t\t\t\t\t\t", "\n\t\t\t\t\t\tthis._fragment.create();\n\t\t\t\t\t"], deindent(_j, options.dev && "if ( options.hydrate ) throw new Error( 'options.hydrate only works if the component was compiled with the `hydratable: true` option' );")), block.hasIntroMethod ? 'intro' : 'mount', (generator.hasComponents || generator.hasIntroTransitions) &&
        "this._flush();", generator.hasComplexBindings &&
        "while ( this._bindings.length ) this._bindings.pop()();", templateProperties.oncreate && (_k = ["\n\t\t\t\tif ( options._root ) {\n\t\t\t\t\toptions._root._oncreate.push( @template.oncreate.bind( this ) );\n\t\t\t\t} else {\n\t\t\t\t\t@template.oncreate.call( this );\n\t\t\t\t}\n\t\t\t"], _k.raw = ["\n\t\t\t\tif ( options._root ) {\n\t\t\t\t\toptions._root._oncreate.push( @template.oncreate.bind( this ) );\n\t\t\t\t} else {\n\t\t\t\t\t@template.oncreate.call( this );\n\t\t\t\t}\n\t\t\t"], deindent(_k)), prototypeBase, proto, name, _set, name, name, templateProperties.ondestroy && "@template.ondestroy.call( this );")));
    var usedHelpers = new Set();
    var result = builder
        .toString()
        .replace(/(\\)?@(\w*)/g, function (match, escaped, name) {
        if (escaped)
    ...
    if (sharedPath) {
        ...
        var names = Array.from(usedHelpers).sort().map(function (name) {
...
var SsrGenerator = (function (_super) {
    ...
    function SsrGenerator(parsed, source, name, options) {
        var _this = _super.call(this, parsed, source, name, options) || this;
        _this.bindings = [];
        _this.renderCode = '';
...
function ssr(parsed, source, options) {
    ...
    var generator = new SsrGenerator(parsed, source, options.name || 'SvelteComponent', options);
    ...
    var result = (_a = ["\n\t\t", "\n\n\t\tvar ", " = {};\n\n\t\t", ".filename = ", ";\n\n\t\t", ".data = function () {\n\t\t\treturn ", ";\n\t\t};\n\n\t\t", ".render = function ( state, options ) {\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\treturn `", "`.trim();\n\t\t};\n\n\t\t", ".renderCss = function () {\n\t\t\tvar components = [];\n\n\t\t\t", "\n\n\t\t\t", "\n\n\t\t\treturn {\n\t\t\t\tcss: components.map( x => x.css ).join( '\\n' ),\n\t\t\t\tmap: null,\n\t\t\t\tcomponents\n\t\t\t};\n\t\t};\n\n\t\tvar escaped = {\n\t\t\t'\"': '&quot;',\n\t\t\t\"'\": '&#39;',\n\t\t\t'&': '&amp;',\n\t\t\t'<': '&lt;',\n\t\t\t'>': '&gt;'\n\t\t};\n\n\t\tfunction __escape ( html ) {\n\t\t\treturn String( html ).replace( /[\"'&<>]/g, match => escaped[ match ] );\n\t\t}\n\t"], _a.raw = ["\n\t\t", "\n\n\t\tvar ", " = {};\n\n\t\t", ".filename = ", ";\n\n\t\t", ".data = function () {\n\t\t\treturn ", ";\n\t\t};\n\n\t\t", ".render = function ( state, options ) {\n\t\t\t",
        ...
        "\n\n\t\t\treturn \\`", "\\`.trim();\n\t\t};\n\n\t\t", ".renderCss = function () {\n\t\t\tvar components = [];\n\n\t\t\t",
        "\n\n\t\t\t",
        "\n\n\t\t\treturn {\n\t\t\t\tcss: components.map( x => x.css ).join( '\\\\n' ),\n\t\t\t\tmap: null,\n\t\t\t\tcomponents\n\t\t\t};\n\t\t};\n\n\t\tvar escaped = {\n\t\t\t'\"': '&quot;',\n\t\t\t\"'\": '&#39;',\n\t\t\t'&': '&amp;',\n\t\t\t'<': '&lt;',\n\t\t\t'>': '&gt;'\n\t\t};\n\n\t\tfunction __escape ( html ) {\n\t\t\treturn String( html ).replace( /[\"'&<>]/g, match => escaped[ match ] );\n\t\t}\n\t"], deindent(_a, hasJs && "[\u2702" + parsed.js.content.start + "-" + parsed.js.content.end + "\u2702]", name, name, JSON.stringify(options.filename), name, templateProperties.data ? "@template.data()" : "{}", name, templateProperties.data
        ...
        return "state." + key + " = @template.computed." + key + "( " + deps
            .map(function (dep) { return "state." + dep; })
            .join(', ') + " );";
    }), generator.bindings.length && (_b = ["\n\t\t\t\tvar settled = false;\n\t\t\t\tvar tmp;\n\n\t\t\t\twhile ( !settled ) {\n\t\t\t\t\tsettled = true;\n\n\t\t\t\t\t", "\n\t\t\t\t}\n\t\t\t"], _b.raw = ["\n\t\t\t\tvar settled = false;\n\t\t\t\tvar tmp;\n\n\t\t\t\twhile ( !settled ) {\n\t\t\t\t\tsettled = true;\n\n\t\t\t\t\t", "\n\t\t\t\t}\n\t\t\t"], deindent(_b, generator.bindings.join('\n\n'))), generator.renderCode, name, generator.css && (_c = ["\n\t\t\t\tcomponents.push({\n\t\t\t\t\tfilename: ", ".filename,\n\t\t\t\t\tcss: ", ",\n\t\t\t\t\tmap: null // TODO\n\t\t\t\t});\n\t\t\t"], _c.raw = ["\n\t\t\t\tcomponents.push({\n\t\t\t\t\tfilename: ", ".filename,\n\t\t\t\t\tcss: ", ",\n\t\t\t\t\tmap: null // TODO\n\t\t\t\t});\n\t\t\t"], deindent(_c, name, JSON.stringify(generator.css))), templateProperties.components && (_d = ["\n\t\t\t\tvar seen = {};\n\n\t\t\t\tfunction addComponent ( component ) {\n\t\t\t\t\tvar result = component.renderCss();\n\t\t\t\t\tresult.components.forEach( x => {\n\t\t\t\t\t\tif ( seen[ x.filename ] ) return;\n\t\t\t\t\t\tseen[ x.filename ] = true;\n\t\t\t\t\t\tcomponents.push( x );\n\t\t\t\t\t});\n\t\t\t\t}\n\n\t\t\t\t", "\n\t\t\t"], _d.raw = ["\n\t\t\t\tvar seen = {};\n\n\t\t\t\tfunction addComponent ( component ) {\n\t\t\t\t\tvar result = component.renderCss();\n\t\t\t\t\tresult.components.forEach( x => {\n\t\t\t\t\t\tif ( seen[ x.filename ] ) return;\n\t\t\t\t\t\tseen[ x.filename ] = true;\n\t\t\t\t\t\tcomponents.push( x );\n\t\t\t\t\t});\n\t\t\t\t}\n\n\t\t\t\t",
        "\n\t\t\t"], deindent(_d, templateProperties.components.value.properties.map(function (prop) {
        var name = prop.key.name;
        var expression = generator.importedComponents.get(name) ||
            "@template.components." + name;
        return "addComponent( " + expression + " );";
    }))))).replace(/(\\)?@(\w*)/g, function (match, escaped, name) { return escaped ? match.slice(1) : generator.alias(name); });
    return generator.generate(result, options, { name: name, format: format });
    var _a, _b, _c, _d;
}

var version$1 = "1.24.0";
